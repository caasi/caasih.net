...
let make = () => {
  ...
  let ref = React.useCallback0((node: Js.Nullable.t<Dom.element>) => {
    ignore(
      %Option({
        let node = Js.Nullable.toOption(node)
        let ctx = CanvasElement.getContextWebGl(node)
        %Option({
          let vert_shader = ctx->GlUtils.prepareVertexShader(_VSHADER_SOURCE)
          %Option({
            let frag_shader = ctx->GlUtils.prepareFragmentShader(_FSHADER_SOURCE)
            %Option({
              let program = ctx->GlUtils.prepareProgram(vert_shader, frag_shader)
